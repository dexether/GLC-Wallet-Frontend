...
@section('title')
    {{$dogecoin->xml_code}}/{{$usd->xml_code}}
@endsection
@section('content')
    <style>
        .order_book {
                ...
                <div class="panel-heading">
                    ...
                    <div class="heading-elements">
                        <span class="heading-text">{{trans_choice('general.last',1)}} {{trans_choice('general.traded',1)}} {{trans_choice('general.price',1)}}
                            : <span class="text-bold text-danger-600 position-right current_dogecoinusd_price">  {{number_format($current_dogecoin,2)}}</span></span>
                    </div>
                </div>
                <div class="panel-body ">
                    <input type="hidden" id="btc_max" value="{{$dogecoin->maximum_amount}}">
                    <input type="hidden" id="btc_min" value="{{$dogecoin->minimum_amount}}">
                    <input type="hidden" id="baalance">
                    {!! Form::open(array('url' => url('market/dogecoinusd/buy'), 'method' => 'post', 'id' => 'buy-form')) !!}
                    <div class="row">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>{{trans_choice('general.lowest_ask',1)}}
                                    :{{number_format($minimum_dogecoin,2)}}</label>
                    ...
                    {!! Form::close() !!}
                    {!! Form::open(array('url' => url('market/dogecoinusd/sell'), 'method' => 'post', 'id' => 'sell-form')) !!}
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>{{$dogecoin->xml_code}} {{trans_choice('general.balance',1)}}
                                    :{{round(\App\Helpers\GeneralHelper::user_dogecoin_balance(Sentinel::getUser()->id)-\App\Helpers\GeneralHelper::user_dogecoin_locked_balance(Sentinel::getUser()->id),$dogecoin->decimals)}}
                                </label>
                                <input type="number" placeholder="{{trans_choice('general.volume',1)}}"
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>{{trans_choice('general.highest_bid',1)}}
                                    :{{number_format($maximum_dogecoin,2)}}</label>
                                <input type="number" placeholder="{{trans_choice('general.price',1)}}"
    ...
    <script>
        ...
        $(document).ready(function (e) {
            var refreshId = setInterval(function () {
                $("#order_book").load('{{url('market/refresh?market=dogecoinusd')}}');
                //$("title").html("yes");
            }, 9000);
        })
    </script>
    ...
    <script>
        ...
        var chart = AmCharts.makeChart("monthly_chart", {
            ...
            "dataSets": [{
                ...
                "color": "#7f8da9",
                "dataLoader": {
                    "url": "{{url('market/graph?market=dogecoinusd')}}",
                    "format": "json"
                },
                "title": "OHLC",
                "categoryField": "date"
        ...
        $(document).ready(function (e) {
            /*var refreshGraph = setInterval(function () {
             $.ajax({
             url: "{{url('market/graph?market=dogecoinusd')}}", dataType: "json", success: function (result) {
