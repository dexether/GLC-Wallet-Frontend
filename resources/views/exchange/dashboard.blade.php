                        ...
                        <li>
                            <div class="onhover-dropdown" >
                                ...
                                <ul class="onhover-show-div" id="price_ticker">
                                    <a class="teading_a" href="{{url('dashboard-exchange',"BTC")}}"><li><img src="{{ url('/') }}/assets/themes/limitless/images/icon/btc.png" alt="">BTC</li></a>
                                    <a class="teading_a" href="{{url('dashboard-exchange',"ETH")}}"><li><img src="{{ url('/') }}/assets/themes/limitless/images/icon/eth.png" alt="">ETH</li></a>
                                    <a class="teading_a" href="{{url('dashboard-exchange',"LTC")}}"><li><img src="{{ url('/') }}/assets/themes/limitless/images/icon/ltc.png" alt="">LTC</li></a>
                                    <a class="teading_a" href="{{url('dashboard-exchange',"XRP")}}"><li><img src="{{ url('/') }}/assets/themes/limitless/images/icon/xrp.png" alt="">XRP</li></a>

                                    {{--<a href="{{url('dashboard-exchange',"BTC")}}"><li   <!-- onclick="call_all('BTC')"--> ><img src="{{ url('/') }}/assets/themes/limitless/images/icon/btc.png" alt="">BTC</li></a>--}}
                                    {{--<a href="{{url('dashboard-exchange',"ETH")}}"><li   <!-- onclick="call_all('ETH')"--> ><img src="{{ url('/') }}/assets/themes/limitless/images/icon/eth.png" alt="">ETH</li></a>--}}
                                    {{--<a href="{{url('dashboard-exchange',"LTC")}}"><li   <!-- onclick="call_all('LTC')"--> ><img src="{{ url('/') }}/assets/themes/limitless/images/icon/ltc.png" alt="">LTC</li></a>--}}
                                    {{--<a href="{{url('dashboard-exchange',"XRP")}}"><li   <!-- onclick="call_all('XRP')"--> ><img src="{{ url('/') }}/assets/themes/limitless/images/icon/xrp.png" alt="">XRP</li></a>--}}
                                </ul>
                            </div>
                        </li>
                                            ...
                                            <table id="order-tbl" class="display" style="width:100%">
                                                ...
                                                @foreach($exchange_data as $exg)
                                                    @if($exg->status == 0 || $exg->status == 2)
                                                        ...
                                                        <tr id="{{"order_cancal".$exg->id}}" style="background-color: <?php echo $color; ?>">
                                                            <td><b>{{$i++}}</b></td>
                                                            <td><b>{{ $exg->side}}</b></td>
                                                            <td>{{ $exg->order_id }}</td>
                                                            <td>{{ $exg->symbol }}</td>
                                                            ...
                                                            <td >
                                                                @if($exg->status == 0)
                                                                    <lable onclick="cancel_order_user('{{$exg->id}}','{{$exg->order_id}}')" class="btn btn-info">cancel</lable>
                                                                @endif
                                                            </td>
                                                            <td>
                                                                @if($exg->status == 0)
                                        ...
                                        <table id="order-his-tbl" class="display" style="width:100%">
                                            ...
                                            @foreach($exchange_data as $exg)
                                                @if($exg->status == 3 || $exg->status == 1)
                                                    ...
                                                    <?php $total = ($exg->price) * ($exg->same_price);  ?>
                                                    <tr onclick="setvalue('{{$exg->same_price}}','{{$exg->price}}')" style="background-color: <?php echo $color; ?>">
                                                        <td><b>{{$i++}}</b></td>
                                                        <td><b>{{ $exg->side}}</b></td>
                                                        <td>{{ $exg->order_id }}</td>
                                                        <td>{{ $exg->symbol }}</td>
...
<script type="text/javascript">
    ...
    function call_all(coin) {
       ...
       /* var type_coin = $('#coin').val();
        ...
        $.ajax({ url:'{{url('')}}/exchange/order_middle1/'+coin,  type: 'get',
            success: function(response) {  $('#order_middle').append(response);  }  });

        $.ajax({ url:'{{url('')}}/exchange/order_full1/'+coin, type: 'get',
            success: function(response) {  $('#order_full').append(response);  }   });

        $.ajax({ url:'{{url('')}}/exchange/order_full_block1/'+coin, type: 'get',
            success: function(response) {  $('#full_block').append(response);  }  });

        $.ajax({ url:'{{url('')}}/exchange/buy_full_coin1/'+coin, type: 'get',
            success: function(response) {  $('#buy_full_coin').append(response);  }  });

        $.ajax({ url:'{{url('')}}/exchange/sell_full_coin1/'+coin, type: 'get',
    ...
    function call_order(option)
    {
        $('#order_middle').empty();
        $.ajax({
            url: '{{url('')}}/exchange/call_order_middle',
            type: 'post',
    ...
    function call_order_full(option)
    {
        $('#order_full').empty();
        $.ajax({
            url: '{{url('')}}/exchange/call_order_full',
            type: 'post',
            data:{ option:option,_token:"{{csrf_token()}}"},
            success: function(response)
            {
...
<script>
    ...
    function callshoket(currentcoin) {
        ...
        $("#data-tab_asks > tbody").empty();
        $("#data-tab_bids > tbody").empty();
        ...
        var socket = io('http://206.189.141.54/');
        var socket = io('http://localhost:3000/');

        socket.on('msg', function (message) {
            var type_coin = $('#coin').val();
            if(message.data.pair == type_coin+':USD') {

                // console.log(currentcoin+":USD");
                countsbids = $('.usersdatabids')
                countsbids = $('.usersdatabids').length;
                countsasks = $('.usersdataasks').length;

                $("table *").delay("slow").removeClass("intro");
                ...
                if (message.data.bids || message.data.asks) {
                    ...
                    if ((message.data.asks.length >= 1) && (message.data.asks[0][1])) {
                        ...
                        var asksrate = 'onclick="setvalue(' + message.data.asks[0][0] * aed_price_admin + ',' + message.data.asks[0][1] + ')"';
                        var markupask = '<tr  id="a' + ((message.data.asks[0][0] * aed_price_admin).toString()).replace(/\./g, '_') + '" data-number="' + message.data.asks[0][0] * aed_price_admin + '" class="usersdataasks" ' + asksrate + ' > <td>' + message.data.asks[0][0] * aed_price_admin + '</td><td>' + message.data.asks[0][1] + '</td><td>' + (message.data.asks[0][0] * aed_price_admin * message.data.asks[0][1]).toFixed(2) + '</td></tr>';

                        var highestNum2 = message.data.asks[0][0];
                        var highestNum = 0;
                        var set = 0;
                        var num = 0;
                        var id = 0;
                        ...
                        $("tbody.askshow .usersdataasks").each(function () {

                            num = this.getAttribute("data-number");
                            if (num == highestNum2 && countsasks >= 0) {
                                // exit;
                            }
                        ...
                        if (bid_set > 0) {
                            old_asks = removeElementsWithValue(old_asks, 0);
                        }
                        var highestNum = Math.max.apply(Math, old_asks);
                        var lovernumber = Math.min.apply(Math, old_asks);

                        // console.log(highestNum+"--"+lovernumber);
                        var near_high = highestNum;
                        var near_low = lovernumber;
                        var new_bid = highestNum2;
                        ...
                        $("tbody.askshow .usersdataasks").each(function () {
                            ...
                            if (new_bid <= num && near_high > num) {
                                near_high = num;
                            }


                            if (num <= new_bid && num > near_low) {
                                near_low = num;
                            }
                        });
                        ...
                        if (id3 != 0 && id4 != 0) {
                            if (id3 < highestNum2) {
                                $('#a' + id3.toString().replace(/\./g, '_')).after(markupask).css("color", "black").addClass("intro");
                            }
                            else if (id3 > highestNum2) {
                                $('#a' + id3.toString().replace(/\./g, '_')).before(markupask).css("color", "black").addClass("intro");
                            }
                        }
                        else if (id3 != 0) {
                            $('#a' + id3.toString().replace(/\./g, '_')).before(markupask).css("color", "black").addClass("intro");
                            ...
                            $('#a' + id4.toString().replace(/\./g, '_')).after(markupask).css("color", "black").addClass("intro");
                        }
                        else {
                            if (countsasks == 0) {
                                $("tbody.askshow").prepend(markupask).css("color", "black").addClass("intro");
                            }
                        }
                        markupask = "";
                    ...
                    /***************** *******************/
                    else if ((message.data.bids.length >= 1) && (message.data.bids[0][1])) {

                        // console.log("bids");

                        var bidsrate = 'onclick="setvalue(' + message.data.bids[0][0] * aed_price_admin + ',' + message.data.bids[0][1] + ')"';
                        var markupbids = '<tr id="b' + ((message.data.bids[0][0] * aed_price_admin).toString()).replace(/\./g, '_') + '" data-number="' + message.data.bids[0][0] * aed_price_admin + '"  class="usersdatabids" ' + bidsrate + ' > <td>' + message.data.bids[0][0] * aed_price_admin + '</td><td>' + message.data.bids[0][1] + '</td> <td>' + (message.data.bids[0][0] * aed_price_admin * message.data.bids[0][1]).toFixed(2) + '</td></tr>';
                        ...
                        $("tbody.bidsshow .usersdatabids").each(function () {
                            ...
                            num = this.getAttribute("data-number");
                            if (num == highestNum1 && countsbids >= 0) {
                                // exit;
                            }
                        ...
                        if (bid_set > 0) {
                            old_bids = removeElementsWithValue(old_bids, 0);
                        }
                        var highestNum = Math.max.apply(Math, old_bids);
                        var lovernumber = Math.min.apply(Math, old_bids);

                        var near_high = highestNum;
                        var near_low = lovernumber;
                        var new_bid = highestNum1;
                        ...
                        if (id != 0 && id2 != 0) {
                            ...
                            if (id < highestNum1) {
                                $('#b' + id.toString().replace(/\./g, '_')).before(markupbids).addClass("intro");
                            }
                            else if (id > highestNum1) {
                                $('#b' + id.toString().replace(/\./g, '_')).after(markupbids).addClass("intro");
                            }
                            ...
                            $('#b' + id2.toString().replace(/\./g, '_')).before(markupbids).addClass("intro");
                        }
                        else {

                            if (countsbids == 0) {
                                $("tbody.bidsshow").append(markupbids).addClass("intro");
                                ;
                            }
                        }
                        markupbids = "";
...
<script>
    ...
    function buy_order(){
        ...
        $.ajax({
            /* the route pointing to the post function */
            url: '{{url(('placeordersbuy'))}}',
            type: 'POST',
            /* send the csrf-token and the input to the controller */
            data: {_token: '{{csrf_token()}}',USD: USD, COIN: BTC ,type:type,order_type:order_type},
            dataType: 'JSON',
            async:true,
            /* remind that 'data' is the response of the AjaxController */
            success: function (data) {
                ...
                {
                    swal('alert?', data.reason, 'question');
                    window.location = "{{url('dashboard-exchange',$coinuse)}}";
                }
...
<script>
    function tradehistory(coin) {
        $.get('{{url('treadehistory')}}'+'/'+coin, function(response){
        ...
        ordersChannelnew = pusher.subscribe(currrent);
            ...
            $.each(['order_created', 'order_changed', 'order_deleted'], function (eventIndex, eventName) {
                ...
                ordersChannelnew.bind(eventName, function (data) {
                    ...
                    placeholder.append('<tr><td>' + eventName + '</td><td> ' + data.amount + '  </td><td> ' + parseFloat((data.price * {{$aed_rate}})).toFixed(4) + ' AED ' + ((data.order_type == 0) ? 'BUY' : 'SELL') + '</td></tr>');
                });
            });
