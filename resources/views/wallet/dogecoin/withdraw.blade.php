...
@section('title')
    {{trans_choice('general.withdraw',1)}}   {{$ltc->name}}
@endsection
@section('content')
<section>
        ...
        <div class="col-md-9">
            <div class="panel panel-white with_drw">
                <div class="panel-heading">
                    <h6 class="panel-title"> {{trans_choice('general.withdraw',1)}}   {{$ltc->name}}</h6>

                    <div class="heading-elements">

                    </div>
                </div>
                {!! Form::open(array('url' => url('wallet/ltc/withdraw/manual'), 'method' => 'post', 'id' => 'withdraw-form',"enctype"=>"multipart/form-data")) !!}
                <input type="hidden" id="network" value="btc">
                <input type="hidden" id="fee_method" value="{{$ltc->fee_method}}">
                <input type="hidden" id="withdrawal_fixed_fee" value="{{round($ltc->withdrawal_fixed_fee,$ltc->decimals)}}">
                <input type="hidden" id="withdrawal_percentage_fee" value="{{round($ltc->withdrawal_percentage_fee,$ltc->decimals)}}">
                <div class="panel-body ">
                    <div class="form-group">
                        {!! Form::label('name',trans_choice('general.label',1),array('class'=>'')) !!}
                        {!! Form::text('name',null, array('class' => 'form-control cust_fld','id'=>'name','placeholder'=>trans_choice('general.crypt_label_placeholder',1))) !!}
                    </div>
                    <div class="form-group">
