...
@section('title')
    {{ trans_choice('general.verify',1) }} {{ trans_choice('general.phone',1) }}
@endsection
@section('content')
    <div class="panel panel-white">
        <div class="panel-heading">
            <h6 class="panel-title"> {{ trans_choice('general.verify',1) }} {{ trans_choice('general.phone',1) }}</h6>
        ...
        <div class="panel-body">
            ...
            @else
                ...
                @else
                    ...
                    @if(!isset($_COOKIE['otpresend_phone']))
                    <button type="submit" class="btn btn-success btn-sm"
                            id="requestOtp">{{trans_choice('general.request',1)}} {{trans_choice('general.otp',1)}}</button>
                    @endif
                    {!! Form::open(array('url' => 'user/verify_phone/check_otp','class'=>'',"enctype" => "multipart/form-data")) !!}
...
@section('footer-scripts')
    <script>
        $("#requestOtp").click(function (e) {
            $.ajax({
                type: 'GET',
                url: '{{url('user/verify_phone/send_otp/'.Sentinel::getUser()->id)}}',
                dataType: 'json',
