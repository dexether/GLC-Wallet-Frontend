                ...
                <table id="data-table" class="table table-striped table-condensed table-hover">
                    ...
                    <?php $__currentLoopData = $buy_data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                     <tr >
                        ...
                        <td >
                            <?php if($key->status == 0): ?> <label class="label label-default buycount"> Pending</label>
                            <?php elseif($key->status == 1): ?><label class="label label-success buycount">  Success </label>
                            <?php elseif($key->status == 2): ?><label class="label label-warning buycount">  Partially </label>
                            <?php elseif($key->status == 3): ?><label class="label label-danger buycount">  Cancel </label>
                            <?php else: ?>
                            <?php endif; ?>
                        </td>
                     </tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            function getMessage() {
                ...
                $.ajax({
                    type: 'get',
                    url: '<?php echo e(url('buycountshow')); ?>'+'/buy/'+ numItems,
